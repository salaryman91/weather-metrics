 ...
 * - tm(단일 시각)의 최근 후보들을 순회, stn=지점 / stn=0(전체) 모두 처리
 * - help=0/1 대응: 헤더 토큰 사용 + 휴리스틱 보강
 * - EUC-KR/UTF-8 자동 디코딩(iconv-lite)
 * - 우선순위: UV-B(지수) → EUV(일사 환산: ÷25 또는 ×40) → 휴리스틱
 * - 행 단위로 여러 열을 시도(결측/EUV=-999 등 회피)
...
const toNum = (s?: string) => {
  ...
  // 기상청 결측/없음: -9/-9.0/-999 등 → 모두 NaN 처리
  return !isFinite(n) || n <= -8.9 ? NaN : n;
};

// ===== 인코딩 =====
async function decodeKR(res: any): Promise<string> {
...
/** help=0/1 헤더 라인에서 토큰 배열 추출(열 갯수와 일치하는 줄 사용) */
function headerTokens(comments: string[], rowLen: number): string[] | null {
  for (const line of comments) {
    ...
    if (!/(UVB|UV\-B|UVA|UV\-A|EUV|UV\s*INDEX|YYMM|TM|TIME|DATE)/i.test(s)) continue;
...
function pickStationIndex(rows: string[][]): number {
  ...
  for (let c=0; c<cols; c++) {
    ...
    for (const r of rows.slice(-60)) {
      ...
      tot++; if (Number.isInteger(v) && v >= 0 && v < 10000) ok++;
    }
...
function tmCandidates(hours = 24): string[] {
  const out: string[] = [];
  const now = new Date(); const base = new Date(now.getTime());
    ...
    for (const tm of tms) {
      ...
      for (const url of urls) {
        ...
        for (let k=rows.length-1;k>=0;k--){
          ...
          const { uv, col, tx, used } = found; // null 가드 이후 구조분해(never 방지)

          // 타임스탬프
          let ts = Math.floor(Date.now()/1000);
          if (iTime >= 0) {
            const t = parseTs(r[iTime]); if (t) ts = t;
          } else if (pairTime) {
            const rawT = `${r[pairTime[0]]}${r[pairTime[1]]}`;
            const t = parseTs(rawT); if (t) ts = t;
          }

          if (DBG) {
            console.log({ pickedRow: r, tm, stn, method: used, col, tx, uvi: uv, ts, iStn, iTime, pairTime, header: toks });
          }
